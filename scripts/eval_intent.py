"""intent 分類のラベル付き評価。9 class の accuracy を確認する。上流の呼び出しが必要。
...
# 末尾の note は「なぜこの問いを入れたか」。境界の 5 組(返金返品 ↔ アフターサービス、
# 苦情 ↔ 配送、商品相談 ↔ 注文、雑談 ↔ その他、人工対応 ↔ 苦情 / その他)を必ず通す。
SAMPLES = [
    ("注文1001の荷物は今どこ?", "配送", ""),
    ("買った商品はもう発送された?", "配送", ""),
    ...
    # 08: 第 9 分類。ここが「その他」へ落ちると fallback_script の固定文で会話が終わり、
    # 08 章で作った create_ticket の確認フローに一度も到達しない(実測 0.55 で「その他」)。
    ("チケットを作って", "人工対応", "境界: 人工対応 ↔ その他"),
    ("担当の方と直接話したいです", "人工対応", "境界: 人工対応 ↔ 苦情"),
    ("こんにちは", "雑談", ""),
    ("今日はいい天気ですね", "雑談", ""),
