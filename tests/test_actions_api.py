...
05 章の苦情の 2 択からチケットを作る経路(spec §6.2)。ボタンを押した行為そのものが
ユーザーの確認なので、ここには確認の往復が無い。

08 章で Agent の create_ticket tool も実際に tickets へ書くようになったが、あちらは
engine の権限ゲートと確認カード(interrupt)を通る別の経路で、こちらの契約は変わらない。
「ユーザーが押していないのにチケットが増えた」という事故は、**この経路では**
